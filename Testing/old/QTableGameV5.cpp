...
public:
    int boxSize = 50;
    int heightBox = windowHeight / boxSize;
    int widthBox = windowWidth / boxSize;
    QTable()
    {
        heightBox = windowHeight / boxSize;
        widthBox = windowWidth / boxSize;
        decisionTable = vector<vector<vector<bool>>>(
            widthBox, vector<vector<bool>>(heightBox, vector<bool>(widthBox, false)));
    }
    ...
    void saveModel(const string &filename)
    {
        ...
        for (int yB = 0; yB < widthBox; ++yB)
        {
            for (int x = 0; x < heightBox; ++x)
            {
                for (int yG = 0; yG < widthBox; ++yG)
                {
    ...
    {
        cout << "Before: " << decisionTable[state[0]][state[1]][state[2]] << endl;
        decisionTable[state[0]][state[1]][state[2]] = !decisionTable[state[0]][state[1]][state[2]];
        cout << "After: " << decisionTable[state[0]][state[1]][state[2]] << endl;
...
void trainQTable(int episodes)
{
    ...
    int boxSize = decisionTable.boxSize;
    ...
    for (int episode = 0; episode < episodes; ++episode)
    {
        ...
        vector<int> state = {(int)bird.y / boxSize, (int)(pipes[0].x - bird.x) / boxSize, (int)(pipes[0].Ygap) / boxSize};
        int steps = 0;
        ...
        while (running && steps < 20000)
        {
            try
            {
                ...
                for (auto &pipe : pipes)
                {
                    ...
                    {
                        nextPipe = &pipe;
                        state[1] = (pipe.x - bird.x) / boxSize;
                        state[2] = pipe.Ygap / boxSize;
                        break;
                    }
                }
                ++steps;
